...
const CP_KEY = "challengesCheckpoint_v1";
...
export function startChallengesAutoSync(opts: {
    ...
    async function pushDirty() {
        ...
        const docs = await opts.db.challenges.findByIds(ids).exec();
        const rows = Array.from(docs.values()).map((doc: any) => ({ newDocumentState: doc.toJSON() }));

        if (rows.length === 0) return;

        const res = await fetch(`${baseUrl}/replication/challenges/push`, {
            method: "POST",
        ...
        if (Array.isArray(json.conflicts) && json.conflicts.length > 0) {
            ...
            try {
                for (const c of json.conflicts) {
                    await opts.db.challenges.upsert(c);
                }
            } finally {
    ...
    async function pull() {
        const res = await fetch(`${baseUrl}/replication/challenges/pull`, {
            method: "POST",
            headers: { "content-type": "application/json" },
        ...
        if (docs.length > 0) {
            ...
            try {
                for (const d of docs) {
                    await opts.db.challenges.upsert(d);
                }
            } finally {
                applyingRemote = false;
            }
    ...
    const sub = opts.db.challenges.$.subscribe((ev: any) => {
        if (stopped) return;
