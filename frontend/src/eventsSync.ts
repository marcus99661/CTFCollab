...
const CP_KEY = "eventsCheckpoint_v1";
...
export function startEventsAutoSync(opts: {
    ...
    async function pushDirty() {
        ...
        const docs = await opts.db.events.findByIds(ids).exec();
        const rows = Array.from(docs.values()).map((doc: any) => ({ newDocumentState: doc.toJSON() }));

        if (rows.length === 0) return;

        const res = await fetch(`${baseUrl}/replication/events/push`, {
            method: "POST",
    ...
    async function pull() {
        const res = await fetch(`${baseUrl}/replication/events/pull`, {
            method: "POST",
            headers: { "content-type": "application/json" },
    ...
    const sub = opts.db.events.$.subscribe((ev: any) => {
        if (stopped) return;
